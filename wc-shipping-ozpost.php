...
Version: 2.1.0 
Copyright: © 2017 VCSWEB (email : support@ozpost.net.au)
...
function init() {         
    ...
    $options = array(
        'enabled','int_disabled','origin_postcode','origin_suburb','store_postcode','subscriptions_email',
        ...
        'parcel_methods','rpp_handling','exp_handling','cod_handling',
        'clicknsend_methods','cnss_handling','cnsb_handling',
        'overseas_parcel_methods','overseas_handling',            
        ...
        'fastway_fwA5blue','fastway_fwA4blue','fastway_fwA3blue','fastway_fwA3orange','fastway_fwA2blue','fastway_fwBox1','fastway_fwBox2','fastway_fwBox3',
        'hx_methods','hx_handling','hx_user','hx_cust','hx_pass','hx_fuellevy',
        ...
        'cpl_methods','cpl_handling','cpl_satchel_handling','cpl_international_handling','cpl_account','cpl_key',
        'cpl_ezy_labels','cpl_metro_labels','cpl_500g_satchel','cpl_1kg_satchel','cpl_3kg_satchel','cpl_5kg_satchel',
        ...
        'mail_days',
        'lead_time', 'deadline',
        'estimated_delivery_format','show_carrier_name',
        ...
        'tax_status','enable_debug','show_parcel', 'show_errors','test_servers') ;
  ...
  if (empty($this->subscriptions_email))
                    $this->subscriptions_email = get_option('admin_email');    // If the subscription email hasn't been set then use the admin email address // 
            }
...
// test for cURL (We use this rather than the WP_Http class as cURL is more portable ans this code gets ported to many different eCommerce systems //
if (!function_exists('curl_init')) {   _e('<p><strong>Error: cURL doesn\'t appear to be installed. Unable to continue.</strong>' ,'woocommerce-ozpost'); exit ; } 
// Test for SimpleXML 
if (!function_exists('simplexml_load_string')) {   _e('<p><strong>Error: SimpleXML doesn\'t appear to be installed. Unable to continue.</strong>' ,'woocommerce-ozpost'); exit ; } 

$storecode = (isset($this->store_postcode) && ($this->store_postcode !== "")) ? $this->store_postcode:$this->origin_postcode; //  Do Subscription expiration check.Disabling this prevents merchant feedback only. It has no effect on the actual subscription status  
if ( false === ($expires = get_transient( 'ozpost_expires' ) ) ) {
    $expires = $this->get_from_ozpostnet("/quotefor.php?flags=expires&host=$this->host&fromcode=".$storecode ) ; 
...
if($err !== 1) {
    $expires = (int)$expires ;           
        if      ($expires > 0)  { _e('<a href="https://shop.ozpost.net/subscriptions/" target=_blank >Subscription</a> expires in '.$expires. ' days','woocommerce-ozpost'); $this->expiration_email_handler($expires) ; } 
        elseif  ($expires < 0)  { _e('Subscription expired '. abs($expires) . ' days ago' ,'woocommerce-ozpost');$this->expiration_email_handler($expires) ;  } 
        else    { _e('Subscription expires TODAY!!' ,'woocommerce-ozpost');  }
}   else    { _e('Error retrieving Subscription data' ,'woocommerce-ozpost');  }
?></h3>
...
// Do ozpost server tests (if indicated)    
 if((isset($_GET['testservers'])) && ($_GET['testservers'] === "1")) { 
     ...
     _e($this->ozpost_network_test(),'woocommerce-ozpost'); $text = 'Refresh'; 
 ?>
  <!--Do the clear button -->  
...
in whish to use the shipping warehouse closest to the customer:    
...
PLEASE NOTE: Make a copy of your modifications as they will be lost and need to be re-applied whenever this module gets updated. 
*/
...
$flags = $apflag = 0 ; 
$maildays = $deadline =  $leadtime = $Osub = $Dsub  =  $fwvars = $tntvars =  $transvars =  $egovars = $stavars = $smsvars = $hxvars = $cplvars = $skpvars = "";
...
if (is_array($this->letter_methods)) { 
 ...
 if(sizeof($allowed_methods > 0)) { $apflag = 1 ;}
}// No letter methods set //  

 if($dest_country === "AU") { 

    if (is_array($this->satchel_methods))       { foreach ($this->satchel_methods as $method)    { $allowed_methods[] = $method ; } $apflag = $apflag | 1 ;}
    if (is_array($this->parcel_methods))        { foreach ($this->parcel_methods as $method)     { $allowed_methods[] = $method ; } $apflag = $apflag | 1 ;}        
    if (is_array($this->clicknsend_methods))    { foreach ($this->clicknsend_methods as $method) { $allowed_methods[] = $method ; } $apflag = $apflag | 1 ;}       
    ...
    if ((is_array($this->fastway_methods)) && ($this->fastway_city != "DIS") && ($this->fastway_city != "")) { 
        ...
        if($this->fastway_fwA5blue > 0 ) {$fwvars .=  "&fwA5blue=".$this->fastway_fwA5blue  ; }
        if($this->fastway_fwA4blue > 0 ) {$fwvars .=  "&fwA4blue=".$this->fastway_fwA4blue  ; }
        if($this->fastway_fwA3blue > 0 ) {$fwvars .=  "&fwA3blue=".$this->fastway_fwA3blue  ; }  
        if($this->fastway_fwA3orange > 0 ) {$fwvars .= "&fwA3orange=".$this->fastway_fwA3orange  ; }
        if($this->fastway_fwA2blue > 0 ) {$fwvars .=  "&fwA2blue=".$this->fastway_fwA2blue  ; }
        if($this->fastway_fwBox1 > 0 ) {$fwvars .=  "&fwBox1=".$this->fastway_fwBox1  ; } 
        if($this->fastway_fwBox2 > 0 ) {$fwvars .=  "&fwBox2=".$this->fastway_fwBox2  ; } 
        if($this->fastway_fwBox3 > 0 ) {$fwvars .=  "&fwBox3=".$this->fastway_fwBox3  ; } 
        } 
    ...
    if ((is_array($this->hx_methods)) && ($this->hx_cust != "")) { 
        foreach ($this->hx_methods as $method) {  $allowed_methods[] = $method ;  }  
        $hxvars = "&HXusername=".$this->hx_user."&HXpassword=".$this->hx_pass."&HXcustomer=".$this->hx_cust ;
  }         
            
}  
    else {  //  Not Australia 
        
        if ($this->int_disabled == 'yes') { return  ; } // No international global setting - clean exit - no errors. We aint even here.  admin setting 
 ...
 if($dest_country) { // do we even have a country? 
    ...
    if ((is_array($this->trd_methods))) { 
        ...
        $transvars = "&TransDirect=1"  ; $transvars .=  "&TRDtype=".$this->trd_type ;      
            if(( $this->trd_email))  $transvars .= "&TRDusername=".$this->trd_email ;
            if(( $this->trd_password )) $transvars .= "&TRDpassword=".$this->trd_password ;
        } 
    ...
    if (is_array($this->cpl_methods)) { 
        ...
        $cplvars = "&CPLacct=".$this->cpl_account."&CPLkey=".$this->cpl_key ;
         ...
         else {$cplvars = "&CPLv3=1" ;} 
         
         if($this->cpl_metro_labels > 0 ) {$cplvars .=  "&CPLml=".$this->cpl_metro_labels ; }
         if($this->cpl_ezy_labels > 0 ) {$cplvars .=  "&CPLel=".$this->cpl_ezy_labels ; }
         if($this->cpl_500g_satchel > 0 ) {$cplvars .=  "&CPLsat0=".$this->cpl_500g_satchel ; }
         if($this->cpl_1kg_satchel > 0 ) {$cplvars .=  "&CPLsat1=".$this->cpl_1kg_satchel ; }
         if($this->cpl_3kg_satchel > 0 ) {$cplvars .=  "&CPLsat2=".$this->cpl_3kg_satchel ; }
         if($this->cpl_5kg_satchel > 0 ) {$cplvars .=  "&CPLsat3=".$this->cpl_5kg_satchel ; }
        } 

if (sizeof($allowed_methods) == 0 ) {
    if (function_exists('wc_add_notice')) {
    wc_add_notice( __( 'Error: Ozpost shipping is active but no methods have been enabled', 'woocommerce-ozpost' ), 'error' ) ; 
    }
    return ; 
}
...
//if($this->show_errors === "yes") $allowed_methods[] = "Error" ; 
   // if (($this->enable_debug === "yes") && (!is_page('checkout'))) { 
       if (($this->enable_debug === "yes") ) {     
            if (function_exists('wc_add_notice')) {
           wc_add_notice( __( "DEBUG (A): Allowed Methods<div><textarea rows=7 cols=100>".print_r($allowed_methods, true)."</textarea></div>", 'woocommerce-ozpost' ), 'notice' ) ;
            }
        }
    ...
    $vars = $cplvars.$smsvars.$stavars.$fwvars.$tntvars.$egovars.$hxvars.$transvars.$skpvars;
...
if ( 'yes' === get_option( 'woocommerce_shipping_cost_requires_address' ) ) {
   if (!$Dsub) {    
       if (function_exists('wc_add_notice')) {
    wc_add_notice( __( 'Accurate shipping costs require a Suburb name', 'woocommerce-ozpost' ), 'notice' ) ; 
       } 
    } // return ; 
}

// default dimensions //
$defaultDimensions = array(0,0,0) ; if ($this->default_dimensions) $defaultDimensions = explode(',',$this->default_dimensions) ;

$index = $dg =  $shippingtaxrate =  0 ; $taxrate = 1  ;
$items = array() ;
 //  get Aust Tax rate 
...
if ($wpdb->num_rows > 0)  { 
    ...
    if ($taxrate <= 0 ) { $taxrate = 1 ;}  // prevent possible division by zero errors /
            ...
            $taxQuery = $wpdb->get_results( "SELECT tax_rate FROM {$wpdb->prefix}woocommerce_tax_rates WHERE tax_rate_country = 'AU' AND tax_rate_shipping = 1 " );
            if ($wpdb->num_rows > 0)  { 
                $shippingtaxrate = $taxQuery[0]->tax_rate  ;}
            }
    }
if ($shippingtaxrate <= 0 ) { $shippingtaxrate = 1 ; } // prevent possible division by zero errors /
if ($shippingtaxrate > 1 ) { $this->tax_status = "taxable" ;}
...
foreach ( $package['contents'] as $item => $values ) {
 ...
 if ( $values['quantity'] > 0 && $values['data']->is_virtual() === false && $values['data']->is_downloadable() === false ) {  $item = $item ;     
        ...
        $variants = ( isset($values['data']->variation_id )) ?  get_post_meta($values['data']->parent->id):false; // and variants
             ...
             $product->weight = ($values['data']->weight) ? $values['data']->weight:$variants['_weight'][0] ;
             $product->length = ($values['data']->length) ? $values['data']->length:$variants['_length'][0] ;
             $product->width =  ($values['data']->width)  ? $values['data']->width :$variants['_width'][0]  ;
             $product->height = ($values['data']->height) ? $values['data']->height:$variants['_height'][0] ;
             $product->price =  ($values['data']->price)  ? $values['data']->price:$variants['_price'][0] ;
    ...
    //    if(($this->enable_debug === "yes" ) && ($product->get_attribute( 'DG' ) == 1)) {  
             if(($product->get_attribute( 'DG' ) == 1)) {   
         wc_add_notice( __( 'Info: Dangerous Goods ID:' . $product->id . ' : ' . $product->description , 'woocommerce-ozpost' ), 'notice' ) ; 
        }     
 ...
 // Save these in case of error - We use them to calculate static rates // 
   $parcelweight += $weight * $values['quantity']; $parcelitems += $values['quantity'] ; $parcelvalue += $price;
...
if ($index == 0) { // No products to quote (all products are virtual or downloads)
             if (function_exists('wc_add_notice')) {
                wc_add_notice( __( 'No shippable products (ozpost mudule)', 'woocommerce-ozpost' ), 'notice' ) ;
    return ;  
}
 ...
 if(((substr($result, 0, 7)) != "<error>") && ($result)) {  
     ...
     if ($xmlQuotes !== false) {
         ...
         $sub =  urldecode((string)($xmlQuotes->information[0]->fromsuburb)) ;
            if ( $this->origin_suburb != $sub )   $this->update_suburb($sub) ; 
             
         $days = intval($xmlQuotes->information[0]->expires);
         
         $this->expiration_email_handler($days) ; 
         
 if (($this->show_parcel === "yes") && $days >= 0 && (!is_page('XcheckoutX'))) {  // Don't display on checkout page because it seems to prevent the page from completely loading. 
        if (function_exists('wc_add_notice')) {
    if ($parcelitems > 1)  {
            wc_add_notice( __( "Parcel Build<div><textarea rows=7 cols=100>".$xmlQuotes->parcel_build."</textarea></div>", 'woocommerce-ozpost' ), 'notice' ) ;
        } else {
            wc_add_notice( __( "Product data<div>" .$xmlQuotes->information[0]->calculated_parcel_weight_kg . "kg, Dims " . $xmlQuotes->information[0]->calculated_parcel_dims_cm ."</div>" , 'woocommerce-ozpost' ), 'notice' ) ;
    }
  }
 } 
if (($this->enable_debug === "yes") && (!is_page('XcheckoutX'))){     
    if (function_exists('wc_add_notice')) {
        wc_add_notice( __( "DEBUG (B): Server returned<div><textarea rows=25 cols=250>".print_r($xmlQuotes, true)."</textarea></div>" , 'woocommerce-ozpost' ), 'notice' ) ;
  }
}
/////////////
  ...
  foreach($xmlQuotes->quote as $quote) {       // Quotes returned
      ...
      if (in_array($quote->id, $allowed_methods)) {  // Continue if an allowed method 
        ...
        switch ($quote->id) {    //  Set the handling fee and/or make custom cost adjustments. Also provides filtering // 
                case "Error"; 
                   if($quote->cost == 0.05)  { wc_add_notice( __( (string)$quote->carrier . " : " .(string)$quote->description, 'woocommerce-ozpost' ), 'error' ) ;  
                   } else { 
                       if ($this->show_errors === "yes") {
                          if (((string)$quote->carrier === "Australia Post") && ($apflag !== 1)) {  break ; }   // Skip if its an Austrlalia Post error and we don't have any of their methods enabled
                         
                            wc_add_notice( __( "<div>Error: ". (string)$quote->carrier . ": " .(string)$quote->description . "</div>", 'woocommerce-ozpost' ), 'error' ) ;  
                       }
                }
                 break;

                case "SLET";case "LL1";case "LL2";case "LL3";   // aust letter
                case "SLETp"; case "LL1p";case "LL2p"; case "LL3p";  // aust priority  
                case "IPLEs"; case "IPLEm";case "IPLEl";  // overseas economy letters 
                case "IPLEppes"; case "IPLEppel" ;  // overseas Economy Prepaid Envelope
                    $handlingFee = $this->letter_handling;
                break;

                case "SLETi";case "LL1i";case "LL2i";case "LL3i"; // aust insured letter 
                case "SLETpi"; case "LL1pi";case "LL2pi"; case "LL3pi";  // aust  insured priority 
                case "IPLEsi"; case "IPLEmi";case "IPLEli";  // overseas economy Insured letters 
                case "IPLEppesi"; case "IPLEppeli" ;  // overseas Economy Prepaid Insured Envelope
                case "REGdl";case "REGb4";  case "REGdli";case "REGb4i"; // aust reg and reg/ins    
                case "IPLRppes"; case "IPLRppel" ;  // Overseas Registered Prepaid Envelope
                    $handlingFee = $this->letter_handling + $this->ri_handling;
                break;


                case "EXLS"; case "EXLM"; case "EXLL";  // aust express letters 
                case "IPLX"; case "IPLXppe";   // Overseas express letters / prepaid express
                case "IPLC"; case "IPLXpps";   // Overseas Courier letters / prepaid Courier
                   $handlingFee = $this->letter_handling + $this->exp_handling;
                break;


                case "EXLSs"; case "EXLMs"; case "EXLLs";  // aust express letters +sig 
                case "EXLSio"; case "EXLMio"; case "EXLLio";  // aust express letters insured (no sig) 
                case "EXLSsi"; case "EXLMsi"; case "EXLLsi";  // aust express letters insured +sig 
                case "IPLXi"; case "IPLXppei";   // Overseas express letters Insured
                case "IPLCi"; case "IPLXppsi";   // Overseas Courier letters Insured / prepaid Courier
...
///////////  parcels aust
                ...
                case "COD"; $handlingFee = $this->cod_handling + $this->ri_handling;break;
...
//  TNT // 
                ...
                case "TNT73i"; $handlingFee = $this->tnt_handling + $this->ri_handling; break;
...
/////////////////////////////////// Fastway 
                ...
                case "FWLbrown"; case "FWLyellow"; case "FWLblack"; case "FWLblue"; 
                case "FWLlime"; case "FWLpink"; 
                case "FWLred"; case "FWLorange"; case "FWLgreen";case "FWLwhite";case "FWLgrey";    
            ...
            case "FWS0":case "FWS1": case "FWS3": case "FWS3l": case "FWS5":
...
//////// Transdirect /////////////////
                // Aust 
            case "TRDmf":case "TRDnl":case "TRDae":case "TRDcp": case "TRDfw": case "TRDtp":case "TRDts":case "TRDti":case "TRDtntr": 
            case "TRDtnt5": case "TRDtnt9": case "TRDtnt10": case "TRDtnt12":
                $handlingFee = $this->trd_handling;
             break;
         
         // Aust insured 
            case "TRDmfi":case "TRDnli":case "TRDaei":case "TRDcpi":case "TRDfwi":case "TRDtpi":case "TRDtsi": case "TRDtii":  case "TRDtntri":
            case "TRDtnt5i": case "TRDtnt9i":  case "TRDtnt10i":case "TRDtnt12i":
                $handlingFee = $this->trd_handling + $this->ri_handling ;
             break;  
         
                // international
             case "TRDtntIntEE": case "TRDtntIntDE": case "TRDtntIntEco": case "TRDtollIntP": case "TRDtollIntD":
                $handlingFee = $this->trd_international_handling ;
             break;
         
                  // international insured 
             case "TRDtntIntEEi": case "TRDtntIntDEi": case "TRDtntIntEcoi": case "TRDtollIntPi": case "TRDtollIntDi":
...
////////////// Hunter Express ///////////////////////////////////////
            case "HXAF":  case "HXRF":                      
                if ($this->hx_fuellevy > 0 ) { 
  
                    $costGST = floatval($quote->cost) / 11 ; 
                    $costex = floatval($quote->cost) - $costGST ; 
                    
                    $levy = $costex * (($this->hx_fuellevy) / 100) ; 
                    
                    $newcostex = $costex + $levy ; 
                    $newcostinc =  $newcostex + ($newcostex / 10) ;
                   
                    $quote->otherfeeIncl =  $levy ; 
                    $quote->otherfeeName =  "Fuel levy " ;  
                    $quote->cost = number_format(ceil(($newcostinc)  * 20) / 20, 2) ;  // round up to neastest 5cent  (20 = 100 / 5) 
                    }         
           $handlingFee = $this->hx_handling;  break;
...
//////////// Startrack ///////////////////////
 ...
 //////////  Couriers Please labels & Domestic parcels //
           case "CPLlab": case "CPLdps": case "CPLdpa": case "CPLons":case "CPLona": case "CPLsds":case "CPLsda":case "CPLdss":case "CPLdsa":
                $handlingFee = $this->cpl_handling;
                break;

 //////////  Couriers Please International parcels //
            case "CPLexp":case "CPLsav":
                $handlingFee = $this->cpl_international_handling;
                break;

// Courier please satchels 
             case "CPL5g":case "CPL1": case "CPL3": case "CPL5":
...
// Heavy Parcel surcharge      
  if(intval($xmlQuotes->information[0]->calculated_parcel_weight_kg)  >=  intval($this->hp_weight)) $handlingFee += (float)$this->hp_surcharge; //  Heavy parcel surcharge // 
...
if (($dest_country == "AU") && ($shippingtaxrate > 1 ))  $cost = $cost - ( $cost / ($shippingtaxrate + 1) ) ;  //  Exclude GST if needed 
...
$details = NULL;
    if  ($this->show_handling_fee === 'yes')    $details = ((float) $handlingFee > 0)           ? $details .= $this->handling_text_pre .      number_format((float)$handlingFee,2) .          $this->handling_text_post : $details;
    if  ($this->show_insurance_cost === 'yes')  $details = ((float)$quote->insuranceIncl > 0)   ? $details .= $this->insurancefee_text_pre .  number_format((float)$quote->insuranceIncl,2) . " Insurance ".$this->insurancefee_text_post : $details;
    if  ($this->show_otherfee === 'yes')        $details = ((float)$quote->otherfeeIncl > 0)    ? $details .= $this->otherfee_text_pre .      number_format((float)$quote->otherfeeIncl ,2) ." ". $quote->otherfeeName . $this->otherfee_text_post : $details;
  ...
  //////////////////////////////////////////    
   ...
   // debugging to show postcodes as a quote  // 
     if ($this->enable_debug === "yes" ) { 
         $osub = preg_split("/=/", $Osub) ;  $dsub = preg_split("/=/", $Dsub) ; 
    $rate = array('id' => "ozpost.debug" ,'label' => "Ozpost Debug: From " . "$fromcode $osub[1]" . " to " . "$topcode $dsub[1]" ,'cost' => 0.00);    
    $this->add_rate($rate);  
     }
   /////////////
   //$aa = sizeof($rate) ;
   if  (sizeof($rate)  == 0 ) { $this->get_static_rates($dest_country) ; }
      ...
      $this->get_static_rates($dest_country) ;
         ...
         $this->get_static_rates($dest_country) ;
          if ($this->enable_debug === "yes" )  _e("<br>Server Error <br>$result" ,'woocommerce-ozpost'); 
            } // Server error	
}   // ! FUNCTION calculate_shipping
...
private function get_static_rates($dest_country) {
 ...
 $cost = ""; 
    switch ($this->cost_on_error_method) {
                    ...
                    case "TBA":
                    $title = $this->tba_text ;    
                    if (!$title) $title = "Please contact us for shipping costs";
      ...
      if  ($cost[0] > 0) { 
          if (($dest_country == "AU") && ($shippingtaxrate > 1 ))  $cost[0] = $cost[0] - ( $cost[0] / ($shippingtaxrate + 1) ) ;  //  Exclude GST if needed 
 ...
 $x = explode(',', $this->static_rates) ;
        ...
        $cost = $rate ;
               if ( $this->cost_on_error_method == "CPK") { $cost = $rate * intval(($parcelweight/1000) + 1) ; }
                if ( $this->cost_on_error_method == "CPI") { $cost = $rate * $parcelitems ; }
...
private function get_table_rate($dest_country) {
        ...
        switch ($this->table_type) {
            case ('PRC'): $calc = $parcelvalue; break;
            case ('WGT'): $calc = $parcelweight / 1000;  break; //kgs
            case ('ITM'): $calc = $parcelitems; break;
        }
        $cost = 0;
...
if ( false === ( $data = get_transient( 'ozpost_'.$hash) ) ) {
 ...
 if (isset($_SERVER['REMOTE_HOST']) && $_SERVER['REMOTE_HOST'] === "sue") { // VCSWEB Local debugging (won't work for anyone else) 
                        curl_setopt($ch, CURLOPT_URL, "https://sue/ozpost" . $request);
                        $data = curl_exec($ch); $error1 = curl_error($ch);
                        ...
                        if (($error1 != "") || ($data == "")) {
                            ...
                            $data = curl_exec($ch);  $error2 = curl_error($ch);
                        }

                        if (($error2 != "") || ($data == "")) {
                            curl_setopt($ch, CURLOPT_URL, "https://svr0." . $domain . $request);
                            $data = curl_exec($ch);  $error3 = curl_error($ch);
                            if (($error3 != "") || ($data == "")) {
                                if (function_exists('wc_add_notice')) {
...
?>
 ...
 private function expiration_email_handler($days) {
  ...
  if (isset($this->settings['email_count'])) { 
         ...
         $message = "Please be advised that your subscription to ozpost.net will expire in " . $days . " Days.<br>Subscriptions can be renewed at <a href=\"https://shop.ozpost.net/subscriptions\">shop.ozpost.net/subscriptions</a>" ;
         $subject =  "Ozpost $days day Subscription Reminder" ;
         if (($days <= 14) && ($days > 0) && ( $t == 0))    { wc_mail( $to, $subject , $message, $headers) ; $this->update_email_counter(1) ; }
        elseif (($days <= 10) && ($days > 0) && ( $t < 2))  { wc_mail( $to, $subject , $message, $headers) ; $this->update_email_counter(2) ; }
        elseif (($days <=  7) && ($days > 0) && ( $t < 3))  { wc_mail( $to, $subject , $message, $headers) ; $this->update_email_counter(3) ; }
        elseif (($days <=  3) && ($days > 0) && ( $t < 4))  { wc_mail( $to, $subject , $message, $headers) ; $this->update_email_counter(4) ; }
        elseif (($days <=  0) && ($t    < 5) && ($t != 0))  { 
            $message = "Your subscription to ozpost.net HAS EXPIRED..<br>Subscriptions can be renewed at <a href=\"https://shop.ozpost.net/subscriptions\">shop.ozpost.net/subscriptions</a>";
                                                              wc_mail( $to, "Ozpost ALERT Subscription Expired", $message, $headers)   ; $this->update_email_counter(5) ; }
        if (($days > 14) && ( $t != 0))   {   $this->update_email_counter(0) ; }// Reset flag for next time
